 ...
 */
const pendingTasks = [];
const taskQueueProcessor = async (task, pendingTasks = []) => {
    if (!task) {
        return
    }

    await task();
    return taskQueueProcessor(pendingTasks.shift(), pendingTasks);
}
...
const run = async (numConcurrentTasks, getTask) => {
    ...
    await Promise.all([...Array(numConcurrentTasks).keys()].map(() => {
        return taskQueueProcessor(pendingTasks.shift(), pendingTasks);
    }));
